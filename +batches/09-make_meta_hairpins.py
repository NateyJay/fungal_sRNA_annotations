...
script_dir = Path("09out-scripts")
script_dir.mkdir(parents=True, exist_ok=True)

Path("09out-batch_outputs").mkdir(parents=True, exist_ok=True)
...
config_file = Path("09-config.txt")
...
for project in projects:
	...
	annotation_file = Path(f"../../metaloci/{abbv}.meta.gff3")

	if not Path(f"../annotations/{project}/", annotation_file).is_file():
		continue
	...
	echo "hairpin..."
	yasma.py hairpin -o . -a align/alignment.bam --annotation_folder tradeoff_{cond} -g {genome} --name meta --annotation_file {annotation_file} --cores 56 -m 600
...
while True:
	...
	exec_file = Path(f"09-command.{i}.sh")
	with open(exec_file, 'w') as outf:
		outf.write(f'''#!/bin/bash
#SBATCH --cpus-per-task=28
#SBATCH --ntasks-per-node 1
#SBATCH --output=09out-batch_outputs/%a.out.txt 
#SBATCH --error=09out-batch_outputs/%a.err.txt 
#SBATCH --array {",".join(map(str,group))}%6

module load bowtie
module load samtools

config=09-config.txt
